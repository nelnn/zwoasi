...
def getPhotos ():
    ...
    if root.run:
        ...
        # s.connect(('169.254.224.228',10001))
        # s.send('rx'.encode())
...
def upload_image(location_name: str, image_path: str):
  ...
  upload_url = "https://210.3.170.231/allsky/upload"    # upload url path (please edit IP address or domain name)
...
def text():
    ...
    Chinese=ImageFont.truetype(str(Path("zwoasi_test_v11.py").parent.absolute()/'source/NotoSansHK-Thin.otf'),55)
    Chinese2=ImageFont.truetype(str(Path("zwoasi_test_v11.py").parent.absolute()/'source/NotoSansHK-Thin.otf'),60)
    ...
    I2 = Image.open(str(Path("zwoasi_test_v11.py").parent.absolute()/'source/SpM_Logo_1000.png'))
    ...
    imgopen.save("allsky.jpg")
    imgopen.close()
    #os.remove(os.path.join(path,str(name)+".jpg"))
    
    # Test upload image
    # upload_image("hkspm",     "/home/pi/Desktop/allsky.jpg")
    upload_image("astropark", "allsky.jpg")
    # upload_image("lmhv",      "/home/pi/Desktop/allsky.jpg")

    #Export data to csv file
    # data = [date_time, sqm, etime]
